...
export function PublicIsbnScanner() {
  ...
  const searchMutation = useMutation({
    mutationFn: async (isbn: string) => {
      const response = await fetch(`/api/public/books/isbn/${isbn}`);
          ...
          <div className="flex justify-center">
            <div className="hidden">
              <SignUpButton mode="modal">
                <button ref={(el) => signUpButtonRef.current = el} />
              </SignUpButton>
            </div>
            <BookCard
              book={scannedBook}
              view="recommendation"
              onAddToLibrary={() => signUpButtonRef.current?.click()}
